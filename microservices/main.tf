...
resource "aws_key_pair" "key" {
  key_name   = "LearnAmazonWebServicesMicroservices"
...
resource "aws_instance" "fortune" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "LC_ALL=C pip3 install --user flask flask-cors boto3",
...
resource "aws_s3_bucket" "www" {
  bucket = "www.microservices.${var.domain_name}"
  acl    = "public-read"
...
resource "aws_s3_bucket" "cdn" {
  bucket = "cdn.microservices.${var.domain_name}"
  acl    = "public-read"
...
resource "aws_cloudfront_distribution" "www" {
  aliases             = ["www.microservices.${var.domain_name}"]
...
resource "aws_cloudfront_distribution" "api" {
  aliases         = ["api.microservices.${var.domain_name}"]
  is_ipv6_enabled = true
...
resource "aws_cloudfront_distribution" "cdn" {
  aliases         = ["cdn.microservices.${var.domain_name}"]
  is_ipv6_enabled = true
...
resource "aws_acm_certificate" "cert" {
  domain_name       = "*.microservices.${var.domain_name}"
